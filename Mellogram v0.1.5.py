...
def static(text):
    global today
    text = text.replace(" ", "_")
    with open("/storage/emulated/0/log.txt", "r") as f:        
        gh = f.read()
        print(gh.split())
    if text not in gh:
        with open("/storage/emulated/0/log.txt", "w+") as f:
            f.write(f"Log in {today}: {text}\n")

def bitcoinh1():
    global bnh1, bitcoinBuy, bitcoinSell
    bitcoinBuy = 0.00000230
    bitcoinSell = 0.00000130
    threading.Timer(180, bitcoinh2).start()

def bitcoinh2():
    global bnh2, bitcoinBuy, bitcoinSell
    bitcoinBuy = 0.00000560
    bitcoinSell = 0.00000370
    threading.Timer(180, bitcoinh1).start()
